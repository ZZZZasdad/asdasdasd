...
title.Parent = frame
local stats = {"Play Time","Name","Quest"}
local statLabels = {}
local colors = {
...
local Objectives = plr.PlayerGui:WaitForChild("Objectives"):WaitForChild("Main")
local function GetFloor()
    for i1,v1 in ipairs(Objectives:GetChildren()) do
        if v1:IsA("Frame") and v1.Name ~= "Take down the Statues" then
            return v1.Name
        end
    end
end
