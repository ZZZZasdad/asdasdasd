...
title.Parent = frame
local stats = {"Play Time", "Username", "Opened Gift"}
local statLabels = {}
local colors = {
...
task.spawn(function()
	while task.wait(1) do
		...
		if statLabels["Username"] then statLabels["Username"].Text = "Username: " .. tostring(plr.Name) end
        if statLabels["Opened Gift"] then statLabels["Opened Gift"].Text = "Opened Gift: " .. tostring((getgenv().OpenedGift or 0)) end
	end
end)
