...
local function cleanMiniHollow(model) if model:IsA("Model") and (model.Name:match("MiniHollow") or model.Name:match("Negative Hollow")) then for _,c in ipairs(model:GetChildren()) do if c.Name ~= "RootPart" and c.Name ~= "Hitbox" then c:Destroy() end end end end
